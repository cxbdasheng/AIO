...
其实光猫不存在单运营商使用，因为芯片处理器都是不能可能定制的，仅能定制软件系统，所以要看 **光猫能不能刷机，能不能改参数**，比如陈大剩现在就是用的电信定制版光猫，但我的宽带是移动 1000M。
### 核心需求分析
...
## 全屋 2.5G 网络覆盖
实现全屋 2.5G 网络覆盖，本质上只要我们购买 2.5G 路由器当主路由即可，前面我们已经完成了 2.5G 光猫的配置，接下来我们来配置路由器，实现全屋 2.5G 网络覆盖。
#### 核心需求分析
...
graph LR
    ...
    B -->|Wi-Fi 6/6E| D[其他家庭终端]
```
内网测试陈大剩使用的 iperf3，iperf3 需要路由器和客户端都安装，不会安装的同学可以去搜搜网上教程，陈大剩内网单向打流命令如下。
```shell
iperf3 -c 192.168.8.1 -p 6868  -u -b 3500M
```
#### 有线 2.5G 测速
![有线 2.5G 测速](https://img.it927.com/aio/246.png)
在有线 2.5G 测速下，单向打流测试下路由器内网速度能达到 2.39Gbits 左右的速度，离 2.5G 只插一点点损耗了。
...
在协商速度 2401 Mbps 下，单向打流测试下路由器 Wi-Fi 6/6E 只能跑早 1.5G 左右的内网带宽，离 2.5G 还差很多。
