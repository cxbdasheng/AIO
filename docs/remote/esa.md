...
在配置 D-NET 前，需要确保目标服务支持 IPv6 访问。**测试时务必使用不同网段的 IPv6 设备进行验证**，以下以博客服务为例：

![博客 IPv6 访问测试](https://img.it927.com/aio/455.png)

???+ warning "重要提示"
    许多用户在测试时使用同网段的 IPv6 地址，发现可以正常访问，误以为配置已完成。但实际部署 ESA 后，外网却无法访问。
    
    **原因说明**：同网段设备访问时，**路由器会直接转发流量**；而外网访问时，路由器防火墙可能会阻止连接，导致访问失败。因此测试阶段必须模拟真实的外网访问场景。
