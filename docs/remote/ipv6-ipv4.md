...
假设家庭服务器已经配置好了 IPv6 [远程穿透 - IPv6 DDNS](ddns.md)，域名是 `home.example.com`，解析到 IPv6 地址。但在外出时会遇到：
...
```mermaid
graph LR
    A[IPv4/IPv6 用户]--IPv4/IPv6--> B[代理服务器]
    B--IPv6--> C[家庭服务器]
        style B fill:#ff9999
```

仔细观察这个结构，会发现这正是目前网上流行的 **Cloudflare CDN** 方案（代理服务器为 CDN 边缘节点）。然而在国内使用 **Cloudflare CDN** 访问速度较慢、延迟较高（除非动态更新 Cloudflare 节点）。
