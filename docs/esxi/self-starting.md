...
| 参数 | 推荐值 | 说明 |
|------|--------|------|
| 默认启动延迟 | 120秒 | 虚拟机之间的启动间隔 |
| 默认停止延迟 | 120秒 | 虚拟机关机等待时间 |
| 停止操作 | 关闭电源 | 虚拟机关机方式 |

### 3. 配置虚拟机启动策略

为每个需要自动启动的虚拟机配置启动策略，其中启动顺序越小越先开，陈大剩以三个软路由为例：
...
=== "设置合理的停止延迟"
    ...
    | 虚拟机类型 | 推荐停止延迟 | 原因 |
    |------------|--------------|------|
    | 软路由 | 120-180秒 | 需要保存配置和连接状态 |
    | 数据库服务 | 300-600秒 | 需要完成事务和数据同步 |
    | Web服务 | 60-120秒 | 需要处理现有连接 |
    | 文件服务 | 180-300秒 | 需要同步文件缓存 |
...
|------------|--------------|----------|
| 软路由 | 0-30秒 | 网络服务启动较快 |
| 数据库服务 | 60-120秒 | 需要完整初始化 |
| Web服务 | 30-60秒 | 依赖数据库服务 |
| 监控服务 | 120-180秒 | 需要其他服务就绪 |
