...
???+ tip "内外网分流优化建议"
    ...
    - **内网访问**：通过路由器 DNS 劫持或 Hosts 文件，将访问域名解析到群晖内网 IP（如 192.168.1.100），实现内网直连
    - **外网访问**：使用 CDN 的 CNAME 解析，通过边缘节点加速访问

    这样可以实现：在家时直连 NAS（速度快、无流量限制），在外时通过 CDN 访问（支持双协议栈、全球加速）。
...
#### 验证访问
在配置 D-NET 前，需要确保目标服务支持 IPv6 访问，按照 [访问测试](#_3) 验证是否可以能访问。
#### 配置 D-NET
...
**配置指南**：请参考 [远程访问 - IPv6 DDNS](../remote/ddns.md) 章节的详细教程。
...
=== "步骤 1：申请 SSL 证书"
    ...
    详细配置请参考：[群晖 SSL 证书配置](ssl-acme.md)
